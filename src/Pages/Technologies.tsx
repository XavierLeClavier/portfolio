...
// Local ForceFn type definition (from react-force-graph-2d types)
type ForceFn<T extends object = object> = {
  (alpha: number): void;
  initialize?: (nodes: NodeObject<T>[], ...args: unknown[]) => void;
  [key: string]: unknown;
};
    ...
    if (fgRef.current) {
      ...
      const win = window as Window & {
        d3?: {
          ...
          forceManyBody: (opts?: object) => unknown;
        }
      };
      // Even stronger repulsion for more spacing
      if (win.d3 && typeof win.d3.forceManyBody === "function") {
        // Increase repulsion strength
        fgRef.current.d3Force(
          "charge",
          win.d3.forceManyBody({ strength: -3500 }) as ForceFn<NodeObject<NodeData>>
        );
      }
      ...
      if (win.d3 && typeof win.d3.forceCollide === "function") {
        fgRef.current.d3Force(
          "collide",
          win.d3.forceCollide(50) as ForceFn<NodeObject<NodeData>>
        );
      }
    }
  }, []);
            ...
            nodeCanvasObject={(node, ctx, globalScale) => {
              if (
                ...
                if (node.type === "tool" && icon) {
                  ...
                  // Draw icon centered, no clipping
                  if (img.complete) {
                  ...
                  // For profile and category, keep circle and fill
                  ctx.beginPath();
                    ...
                    <div className="mt-4">
                      ...
                      <ul className="list-disc list-inside flex flex-col gap-1">
                        {relatedProjects.map(projectName => (
                          <Link to={`/projects/${projectName}`} key={projectName} className="text-blue-400 hover:underline">{projectName}</Link>
                        ))}
                      </ul>
                    </div>
                  )}
